...
- **Alışkanlıklar + Virdlerim tab sistemi:** `GunlukTakipler` ekranı `NestedScrollView` ile yapılandırıldı. Namaz kartı `headerSliverBuilder`'da `SliverToBoxAdapter`, TabBar `SliverPersistentHeader(pinned: true)` ile yapışık kalır. `TabBarView` doğrudan `body`. Her iki tab widget'ı (`HabitTrackerWidget`, `VirdlerimContentWidget`) `AutomaticKeepAliveClientMixin` kullanır — tab geçişinde state korunur, veri yeniden çekilmez. Son açık tab `SharedPreferences` ile hatırlanır (`takipler_tab_index`).